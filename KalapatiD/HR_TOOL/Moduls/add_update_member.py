...
class AddOrUpdate:
    ...
    def get_product_name_from_user():
        ...
        print("*" * 50 + "\nSELECTED MENU: Add or update member.")
        product_name = input("Please add the name of the member: ")
    ...
    def get_product_value_from_user():
        ...
        while True:
            try:
                product_value = int(input("Please add the phone number of the member: "))
            except ValueError:
            ...
            if type(product_value) != int:
                ...
                print(f"SUCCESSFUL ADDITION\nThe new member is:  and its value is: {product_value}\n"
                      + "*" * 50
                      )
            break
    ...
    def decide_upgrading(self, product_name):
        ...
        if product_name in product_name_list:
            to_be_updated = True
            print("*" * 50 + "\nMember already exists in database. UPDATING product in database")
        else:
            to_be_updated = False
            print("*" * 50 + "\nMember is not in database. CREATING new product in database")
